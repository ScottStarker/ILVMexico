...
class Session
{
   ...
   function login($subuser, $subpass, $subremember){
      ...
      if(!$subuser || strlen($subuser = trim($subuser)) == 0){
         ...
         if(0 == preg_match("/^([0-9a-z])*$/i", $subuser)){
	  ...
	  $result = $database->confirmUserPass($subuser, $subpass);							// Scott Starker - 3/27/2017
   ...
   function register($subuser, $subpass, $subemail){
      ...
      if(!$subpass){
         ...
         else if(0 == preg_match("/^([0-9a-z])+$/", ($subpass = trim($subpass)))){
      ...
      /* No errors, add the new account to the */
		 ...
		 if($database->addNewUser($subuser, $subpass, $subemail)){									//  Scott Starker - 4/17/2017
//            if(EMAIL_WELCOME){
   ...
   function editAccount($subcurpass, $subnewpass, $subemail){
      ...
      if($subnewpass){
         ...
         if(!$subcurpass){
			...
			if($database->confirmUserPass($this->username, $subcurpass) != 0){						//  Scott Starker - 4/17/2017
               $form->setError($field, "* Current Password incorrect");
            }
         }
...
//        $database->updateUserField($this->username,"password",password_hash($value, PASSWORD_BCRYPT, $options)); 
   ...
   function isAdmin(){
      return ($this->userlevel == ADMIN_LEVEL ||
              $this->username  == ADMIN_NAME || $this->username  == ADMIN_Loren);
   }
   ...
   function generateRandID(){
      //return md5($this->generateRandStr(16));
		...
		return password_hash($this->generateRandStr(16), PASSWORD_BCRYPT, $options);				// password_hash - 4/18/17
   }
